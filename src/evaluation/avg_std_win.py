...
def evaluate_model(env, model, num_episodes=500, record_interval=5, max_steps=1000):
...
def main():
    ...
    all_rewards, total_rewards = evaluate_model(env, model, num_episodes)

    # Calculate statistics
    avg_reward = np.mean(total_rewards)
    std_reward = np.std(total_rewards)
    episodes_over_200 = sum(1 for r in total_rewards if r >= 200)

    print("\nEvaluation Report:")
    print(f"Average Reward: {avg_reward:.2f}")
    print(f"Standard Deviation of Rewards: {std_reward:.2f}")
    print(f"Number of Episodes with Reward >= 200: {episodes_over_200}/{num_episodes}")
    ...
    output_path = os.path.join(output_dir, "best_worst_mean_rewards.csv")
